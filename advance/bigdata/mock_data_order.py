...
browsers = ['Chrome', 'FireFox', 'Internet Explorer', 'BaiDu', 'Other', 'Opera',
            'Safari', 'SouGou', 'UC', ' Microsoft Edge', 'QQ']
genders = ['男', '女', '未知']
medias = ['display', 'video']
device_types = ['PC', 'Mobile', 'Other']
province_list = ['北京市', '天津市', '上海市', '重庆市', '河南省', '安徽省', '福建省', '甘肃省', '贵州省',
                 '海南省', '河北省', '黑龙江省', '湖北省', '湖南省', '吉林省', '江苏省', '江西省', '辽宁省',
                 '青海省', '山东省', '山西省', '陕西省', '四川省', '云南省', '浙江省', '台湾省', '广东省',
                 '广西壮族自治区', '内蒙古自治区', '宁夏回族自治区', '西藏藏族自治区', '新疆维吾尔自治区', '香港',
                 '澳门']
province_city = {
    '北京市': ['北京市'],
    '天津市': ['天津市'],
...
campaigns = ['CC80013', 'CC80012', 'CC80011', 'CC80014', 'CC80015', 'CC80016',
             'CC80017', 'CC80018', 'CC80019', 'CC80020', 'CC80021', 'CC80022',
             'CC80023', 'CC80024']
...
def mock_user(seq, dt):
    ...
    for cycle in range(0, 100):
        ...
        for num in range(0, 50):
            ...
            ssn = faker.ssn()
            job = faker.job()
            user_profile.append('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s' % (
            seq, user_id, user_name, address, sex, email, phone, birthdate, ssn,
            job, dt))
            seq = seq + 1
...
def mock_tracking(seq, dt):
    for cycle in range(0, 100):
        ...
        for num in range(0, 10):
            ...
            user_tracking.append('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s' % (
            seq, mac, mac_md5, ip, tracking_url, uri_extension, media_type,
            device_type, campaign, province, city, dt))
            seq = seq + 1
    f = open('test2.txt', 'a+')
    f.write('\n' + '\n'.join(user_tracking))
...
def mock_order(seq, dt, row_cnt):
    ...
    with open(
            '/home/dataservice/workspace/mock/result/seq/order/2019/test_order_%s.csv' % dt,
            'w') as f:
        for num in range(1, row_cnt + 1):
            ...
            seq, order_id, order_status, pay_level, pay_from, is_appraise,
            appraise_level, deliver_company, province, city, order_time, dt))
            seq += 1
            if num % 10000 == 0:
                f.flush()
